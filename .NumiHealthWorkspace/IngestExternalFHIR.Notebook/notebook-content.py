        ...
        if "name" in activity_data and "msft_fhir_ndjson_bronze_ingestion" in activity_data["name"]:
            if "parameters" in activity_data:
                ...
                if "source_path_pattern" in parameters:
                    ...
                    updated_path = current_path.replace('/Files/Process/Clinical/FHIR-NDJSON', '/Files/External/Clinical/FHIR-NDJSON')
                ...
                if "move_failed_files_enabled" in parameters:
                    parameters["move_failed_files_enabled"] = "false"
                    print(f"Set move_failed_files_enabled to false for activity {activity_id}")

                # Set compression_enabled to "false"
                if "compression_enabled" in parameters:
                    parameters["compression_enabled"] = "false"
                    print(f"Set compression_enabled to false for activity {activity_id}")
        else:
